...
export class ElectricitySmartMeterListComponent implements OnInit , OnDestroy{
  ...
  checkForDisplayingUtilityAndSolarColumn(){
   const displayExtraColumn = this.usageHistoryData.content.find(data =>{
    ...
    if(displayExtraColumn){
      this.dataSource =  this.usageHistoryData.content.map(gasData =>{
        const data = {...gasData};
        ...
        data.total = data.total.toFixed(3);
        return data;
      });
      ...
      this.keys = TableColumnData.SMART_METER_KEYS;
      this.dataSource = this.usageHistoryData.content.map((gasData,index) =>{
        const data = {...gasData};
  ...
  validateYearChange(event: Event) {
    AppUtility.validateChange(event, this.electricitySmartMeterForm.controls['year'], 1, 2100);
  }
  ...
  validateMonthChange(event: Event) {
    AppUtility.validateChange(event, this.electricitySmartMeterForm.controls['month'], 1, 12);
  }
}
