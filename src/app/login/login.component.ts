...
export class LoginComponent implements OnInit {
  ...
  theme: string;
  code: string;
  constructor(private router: Router, private route: ActivatedRoute, private _http: Http, private loginService: LoginService) {
  ...
  login() {
    ...
    if (this.users.username == undefined || this.users.username.length == 0) {
      ...
      let body = "username=" + this.users.username + "&password=" + this.users.password + "&client_id=" + this.users.username + "&client_secret=" + this.users.password + "&grant_type=password&scope=read";
      document.getElementById("loader").classList.add('loading');
      this.loginService.performOauthToken("oauth/token", body).subscribe(data => {
  ...
  postGetCustomeData(userId) {
    this.loginService.performPostMultiPart('customers/current/' + userId).subscribe(
      data => {
        let response = JSON.parse(JSON.stringify(data));
      ...
      error => {
        let response = JSON.parse(JSON.stringify(error));
        console.log(response);
        this.errorMessage = response.error.errorMessage + "<br>Your new account registration is in process.";
        document.getElementById("loader").classList.remove('loading');
      }
    );
  ...
  getAllSurvey(customerId) {
    ...
    this.loginService.performGetMultiPartData("customers/" + customerId + "/surveys").subscribe(
      data => {
        let response = JSON.parse(JSON.stringify(data));
        if(response.errorMessage==null){
